...
register: _requires-sandbox-up  ## Register Flyte cookbook workflows
	...
	REGISTRY=ghcr.io/flyteorg VERSION=latest $(call RUN_IN_SANDBOX,make -C cookbook/$(EXAMPLES_MODULE) serialize)
...
fast_register: _requires-sandbox-up  ## Fast register Flyte cookbook workflows
	...
	REGISTRY=ghcr.io/flyteorg VERSION=latest $(call RUN_IN_SANDBOX,make -C cookbook/$(EXAMPLES_MODULE) fast_serialize)
