...
By default, flytekit binds a single container image, i.e., the `default Docker image <https://ghcr.io/flyteorg/flytekit>`__, to all tasks.
...
# #. ``version`` refers to the tag of the image. For example: latest, or python-3.9 etc. If ``container_image`` is not specified, then the default configured image for the project is used.
#
