...
#
# If auto-scaling group (ASG) is set up, you may want to isolate the tasks that you want to trigger on spot/preemptible instances from the regular workloads.
...
# 
# .. note::
#   In the case your task gets preempted, Flyte will retry your task on a non-spot (regular) instance. This retry will not count towards a retry that a user sets.
#
# Which Tasks Should Be Set To Interruptible?
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# Most Flyte workloads should be good candidates for spot instances. 
# If your task does NOT exhibit the following properties, then you can set ``interruptible`` to true.
#
...
#
# In a nutshell, you should use spot/preemptible instances when you want to reduce the total cost of running jobs at the expense of potential delay in execution due to restarts.
