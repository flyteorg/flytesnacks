...
def prepare_google_trend(google_trend_csv):
...
def prepare_df(
    df, store_csv, weather_csv, state_names_csv, store_states_csv, google_trend_csv
):
    num_rows = df.count()
    ...
    google_trend_all = prepare_google_trend(google_trend_csv=google_trend_csv)
...
def train_model(
    categorical_cols, continuous_cols, hp: Hyperparameters, train_df, len_vocab
):
    ...
    embeddings = [
        Embedding(len_vocab[col], 10, input_length=1, name="emb_" + col)(inputs[col])
    ...
    return keras_model, hp.local_checkpoint_file
...
def horovod_train_task(
    ...
    return (
        ...
        test_model(hp=hp, test_df=test_df, keras_model=keras_model),
    )
