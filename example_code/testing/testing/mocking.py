...
# This is a generic SQL task (and is by default not hooked up to any datastore
# nor handled by any plugin), and must be mocked.
sql = SQLTask(
    "my-query",
...
# Without a mock, calling the workflow would typically raise an exception,
# but with the `task_mock` construct, which returns a `MagicMock` object,
# we can override the return value.
def main_1():
...
# %% [markdown]
# There is another utility as well called `patch` which offers the same
# functionality, but in the traditional Python
